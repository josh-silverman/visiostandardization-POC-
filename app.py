...
# # STEPS = [
# #     ("Extract VSDX and upload XML", run_extract_xml),
# #     ("Parse Visio XML to JSON", run_parse_visio),
# #     ("AI Standardize the JSON", run_ai_standardize),
# #     ("Write JSON back to XML", run_json_to_xml),
# #     ("Create new VSDX", run_create_vsdx),
# # ]
...
# # def main():
# #     st.set_page_config(page_title="Visio VSDX Pipeline", page_icon="🗂️", layout="centered")
# #     st.title("🗂️ Visio VSDX Processing Pipeline")
...
# #         st.sidebar.checkbox(f"{'✅' if completed[idx] else '⬜️'} {desc}", value=completed[idx], key=f"sidebar_{idx}", disabled=True)
...
# #     run_step_buttons = []
# #     for idx, (desc, _) in enumerate(STEPS):
# #         run_step_buttons.append(step_cols[idx].button(f"Run Step {idx+1}", key=f"runstep_{idx}"))
...
# #     # Individual steps
# #     for idx, pressed in enumerate(run_step_buttons):
# #         if pressed:
# #             run_step(idx)
...
# #     st.caption("Built with Streamlit. All steps can be run individually or together. See logs above for status/errors.")

# # if __name__ == "__main__":
# #     # Streamlit requires this to be run as "streamlit run frontend.py"
# #     main()
...
#     # Only allow running all steps, not individual steps
#     if run_all:
#         for idx in range(len(STEPS)):
#             if not st.session_state.get(step_key(idx), False):
#                 run_step(idx)
